...
window.addEventListener("message", (event) => {
    console.log("[5.js] message received", event.data);
...
if (window.electron) {
    console.log("[5.js] window.electron is available, setting up IPC listener.");

    window.electron.receive("update-content", (data) => {
        console.log("[5.js] Received data via IPC:", data);
    ...
    console.warn("[5.js] window.electron is undefined. Skipping IPC listener setup.");
}

const gameid = document.getElementById('gameid');
if (gameid) {
    gameid.innerHTML = "Undefined";
} else {
    console.error("[5.js] 'gameid' element not found.");
}
