...
### v0.8.0 の主な更新点
- **Google スプレッドシートのリンク読み込み**に対応  
- **句データ（phrase）リセット**ボタンを追加  
- データリセット周りの改善、セキュリティや UI の微修正
...
2. **v0.5 系以降**をご利用の場合は**自動更新**で v0.8.0 に更新されます（更新後に再起動）。  
...
### 5.3 Google スプレッドシートからの読み込み（v0.8.0）
1. **共有設定の変更**：  
...
## 11. 表示・操作設定
- **表示サイズ調整**：  
  - `Ctrl` + `-`（縮小）／ `Ctrl` + `Shift` + `+`（拡大）／ `Ctrl` + `0`（リセット）  
...
- **句データリセット**：**句データ（大会名・兼題・句一覧）のみ初期化**（v0.8.0で追加）  
