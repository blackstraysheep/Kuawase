...
### v0.13.0 の主な更新点
- **v0.13.0 リリース対応**に伴う全体的なメンテナンスと安定性向上
- **BGM挙動とExcel入力処理の改善**により、運営時の操作をより安定化
- **依存関係（Electron など）と npm install フローの更新**による導入・保守性の向上
- **ウィンドウ名・アプリ名表示、スプラッシュ表示、フォントパス挙動**を近年リリースで改善
- Google スプレッドシート読み込み、句データリセット、カスタムCSSテーマ機能を継続サポート
...
## 3. インストール／アップデート
1. GitHub Releases から最新のインストーラ（`.exe`）をダウンロードして実行します。  
2. 既存ユーザーは、新バージョン公開時に**自動更新**を利用できます（更新後に再起動）。  
3. 更新やインストールに失敗する場合は、旧版をアンインストールして最新インストーラを再実行してください。
...
#### 左下：試合指定パネル（設定）
- **試合指定パネル（左下）**  
  - 紅チーム／白チーム／兼題（テーマ）／試合名 を指定して保存  
  - **試合名**には HTML タグ（例：`<br>`）が使用可能  
  - 保存すると、投影画面の対戦情報に即時反映されます

#### 右側：進行・表示を制御するパネル

##### 基本操作（対戦切り替え・披講）
- **対戦切り替え**  
  - 大会TOP／試合TOP／先鋒戦／次鋒戦／中堅戦／副将戦／大将戦  
...
- **試合トップ（試合TOPページ）**：  
  - 上部：大会名  
  - 中央上：試合名（例：「Ａブロック 第１試合」）  
  - 中央：兼題名  
  - 下部：「試合TOP」（English: "Match TOP"）  
  - 左右：紅・白チーム名  
  - 左右端：紅・白の句を小さく表示  
- **先鋒戦～大将戦**：  
  - 基本レイアウトは試合トップと同様  
...
## 7. 表示操作（ステージ切替と披講）
- **対戦切り替え**で投影側のページ（大会TOP／試合TOP／各試合）を切り替え  
- **披講機能**：  
...
- **言語切り替え**：  
  ...
  - 例：大会TOP ↔ Compe TOP、試合TOP ↔ Match TOP、  
...
## 12. 保存・リセットの種類と挙動
- **試合指定の保存**：管理画面の試合指定パネルで保存すると、投影表示や内部設定に即座に反映  
- **句データリセット**：**句データ（大会名・兼題・句一覧）のみ初期化**  
...
### Q. 披講ボタンが表示されない
**A.** 披講ボタンは先鋒戦～大将戦でのみ表示される仕様です。試合TOPや大会TOPでは表示されません。
...
- **Excel／Google Sheetsが読み込めない**  
  ...
  - いったんアプリ再起動のうえ再試行  
- **ビルド・インストール関連問題（v0.13.x 時点の改善）**
  - 近年リリースで依存関係とパッケージングフローを更新  
  - インストール時に問題が発生した場合は、旧版をアンインストール後に最新版インストーラーを再実行  
- **投影側が更新されない**  
  - 管理側でステージを再切替してみる  
- **披講ボタンが表示されない**  
