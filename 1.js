...
window.addEventListener("message", (event) => {
    console.log("[1.js] message received", event.data);
    ...
    } else if (data.type === "red" && data.content) {
        document.getElementById("redHaiku").innerHTML = Object.values(data.content)[0] || "error-1.js";
    } else if (data.type === "white" && data.content) {
        document.getElementById("whiteHaiku").innerHTML = Object.values(data.content)[0] || "error-1.js";
    }    
});

// window.electron が存在する場合のみ処理を実行
if (window.electron) {
    console.log("[1.js] window.electron is available, setting up IPC listener.");

    window.electron.receive("update-content", (data) => {
        console.log("[1.js] Received data via IPC:", data);
        ...
        if (data.type === "title" && data.content) { 
            ...
            document.getElementById("redHaiku").innerHTML = Object.values(data.content)[0] || "";
        } else if (data.type === "white" && data.content) {
            document.getElementById("whiteHaiku").innerHTML = Object.values(data.content)[0] || "";
        }    
    });
} else {
    console.warn("[1.js] window.electron is undefined. Skipping IPC listener setup.");
}
...
if (gameid) {
    ...
    console.error("[1.js] 'gameid' element not found.");
}
